...
describe('generateAgentEmail', () => {
  ...
  it('should handle agent ID with special characters', () => {
    expect(generateAgentEmail('my-agent_123', 'mail.example.com')).toBe(
      'my-agent_123@mail.example.com'
    );
  });
...
describe('formatEmailAddress', () => {
  ...
  it('should format address with name', () => {
    expect(formatEmailAddress({ address: 'test@example.com', name: 'John Doe' })).toBe(
      'John Doe <test@example.com>'
    );
  });

  it('should handle undefined name', () => {
    expect(formatEmailAddress({ address: 'test@example.com', name: undefined })).toBe(
      'test@example.com'
    );
  });

  it('should handle empty string name', () => {
    expect(formatEmailAddress({ address: 'test@example.com', name: '' })).toBe(
      'test@example.com'
    );
  });
});
